...
const Waves: React.FC<WavesProps> = ({
  ...
  useEffect(() => {
    ...
    function tick(t: number) {
      ...
      container.style.setProperty('--x', `${mouse.lx}px`);
      container.style.setProperty('--y', `${mouse.ly}px`);

      movePoints(t);
      drawLines();
      if (running) frameIdRef.current = requestAnimationFrame(tick);
    }
    ...
    // Pause animation when tab hidden or scrolled out of view
    const onVisibility = () => {
      if (document.hidden) {
        running = false;
      ...
      } else if (!prefersReducedMotion && running === false && frameIdRef.current === null) {
        running = true;
        frameIdRef.current = requestAnimationFrame(tick);
      }
    };
